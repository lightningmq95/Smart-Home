...
if len(existing_files) >= 175 and not missing_numbers:
    print(f"All videos for gesture {selected_gesture} are already recorded.")
    exit()

start_video_num = len(existing_files) + 1
...
def record_video(gesture, video_num):
    print(f'Prepare to start recording video {video_num} for gesture {gesture}')
    ...
    print(f'Recording video {video_num} for gesture {gesture}')
    video_writer = cv2.VideoWriter(
        os.path.join(gesture_dir, f'{gesture}_clip_{video_num}.mp4'),
    ...
    print(f'Video {video_num} for gesture {gesture} completed')

for missing_num in missing_numbers:
    record_video(selected_gesture, missing_num)

for video_num in range(start_video_num, start_video_num + 175 - len(existing_files)):
    record_video(selected_gesture, video_num)
